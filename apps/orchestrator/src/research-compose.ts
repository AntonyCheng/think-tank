...
function renderResearchProfileContract(
  ...
  return [
    ...
    "- deep 只用于需要多轮追问或递归探索的复杂任务；必须同时声明 breadth、depth、concurrency，不能只写 mode。",
    ...(capabilities.deepResearch
      ? [
          `- deep 部署上限：breadth ${capabilities.deepResearch.maxBreadth}，depth ${capabilities.deepResearch.maxDepth}，预计研究调用数 ${capabilities.deepResearch.maxResearchCalls}。`,
        ]
      : []),
    "- synthesis 只用于汇总、对比、审查或最终报告步骤；该步骤必须声明 depends_on。",
    "- synthesis 步骤的 task 必须用 {{变量名}} 引用至少一个上游 output 变量；它只复用这些材料，不重新联网检索。",
    "- 如需为普通专家步骤覆盖研究限额，只能使用 step.llm.params.think_tank：",
    "  llm:",
    "    params:",
    "      think_tank:",
    "        limits:",
