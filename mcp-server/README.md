...
The server only calls the authenticated task routes under `THINK_TANK_API_URL`; it never reads SQLite or contacts the researcher service directly. `THINK_TANK_SERVICE_API_KEY` must match the main project's `ORCHESTRATOR_SERVICE_API_KEY`.
...
Set `MCP_TRANSPORT=stdio`. The MCP client starts the server as a child process, so the MCP transport itself needs no API key. The internal `THINK_TANK_SERVICE_API_KEY` is still required when browser login is enabled on the main API.
...
This configuration uses HTTP for a trusted internal network. `MCP_API_KEY` authenticates the remote Agent at the MCP boundary; `THINK_TANK_SERVICE_API_KEY` separately authenticates the MCP server when it calls the main task API. Redact `api_key` from any service or reverse-proxy logs.
